...
namespace ToxCore {
	 ...
	 *
	 ******************************************************************************/
	[CCode (cname = "TOX_CORE_CONSTS", cprefix = "TOX_CORE_CONSTS_")]
	namespace Consts {
		/**
	 ...
	 *
	 ******************************************************************************/
	[CCode (cname = "TOX_CORE_ENUMS", cprefix = "TOX_CORE_ENUMS_")]
	namespace Enums {
		/**
	 ...
	 *
	 ******************************************************************************/
	[CCode (cname = "TOX_CORE_TOX_ERRORS", cprefix = "TOX_CORE_TOX_ERRORS_")]
	namespace ToxErrors {
	 ...
	 *
	 ******************************************************************************/
	[CCode (cname = "TOX_CORE_BINDING_ERRORS", cprefix = "TOX_CORE_BINDING_ERRORS_")]
	namespace BindingErrors {
	...
	public class Options {
		 ...
		 */
		private static Options? options_new (out ToxErrors.TOX_ERR_OPTIONS_NEW error);
		public static Options? create ()
			throws OptionError
		{
			ToxErrors.TOX_ERR_OPTIONS_NEW error;
			Options opts = options_new (out error);

			if (error == ToxErrors.TOX_ERR_OPTIONS_NEW.MALLOC) {
				throw new BindingErrors.OptionError.NOMEM ("Unable to allocate memory for Tox.Options object.");
			}
     ...
     */
		public Enums.ProxyType proxy_type;
     ...
     */
		public Enums.SaveDataType savedata_type;
	...
	public class Tox {
		 ...
		 */
		[CCode (cname = "tox_new")]
		private Tox? tox_new (Options? options = null, out ToxErrors.TOX_ERR_OPTIONS_NEW error);
		public Tox? create (Options? options = null)
			throws BindingErrors.ConstructError
		{
			ToxErrors.TOX_ERR_OPTIONS_NEW error;
			Tox? handle = tox_new (options, out error);

			switch (error) {
				case ToxErrors.TOX_ERR_OPTIONS_NEW.NULL:
					throw new BindingErrors.ConstructError.UNKNOWN ("A parameter was null");
					break;
				case ToxErrors.TOX_ERR_OPTIONS_NEW.MALLOC:
					throw new BindingErrors.ConstructError.UNKNOWN ("Unable to allocate memory for the Tox instance");
					break;
				case ToxErrors.TOX_ERR_OPTIONS_NEW.PORT_ALLOC:
					throw new BindingErrors.ConstructError.BAD_PORT_ALLOC ("%s: Unable to seize the received port".printf (error.to_string ()));
					break;
				case ToxErrors.TOX_ERR_OPTIONS_NEW.PROXY_BAD_HOST:
					throw new BindingErrors.ConstructError.BAD_HOST ("Invalid host (host was %s)".printf ((options.proxy_host ?? "null")));
					break;
				case ToxErrors.TOX_ERR_OPTIONS_NEW.PROXY_BAD_PORT:
					throw new BindingErrors.ConstructError.PORT_UNAVAILABLE ("Port not available (%s)".printf (options.proxy_port.to_string ()));
					break;
				case ToxErrors.TOX_ERR_OPTIONS_NEW.PROXY_NOT_FOUND:
					throw new BindingErrors.ConstructError.PROXY_NOT_FOUND ("Unable to connect to the proxy (TYPE_%s : %s)".printf (options.proxy_type.to_string (), options.proxy_host ?? "null"));
					break;
				case ToxErrors.TOX_ERR_OPTIONS_NEW.LOAD_ENCRYPTED:
					throw new BindingErrors.ConstructError.ENCRYPTED_DATA ("Data was unexpectedly encrypted or looks encrypted");
					break;
				case ToxErrors.TOX_ERR_OPTIONS_NEW.LOAD_BAD_FORMAT:
					throw new BindingErrors.ConstructError.MALFORMED_DATA ("Data was not properly formatted");
					break;
				default:
					throw new BindingErrors.ConstructError.UNKNOWN (error.to_string ());
					break;
			}
		...
		public uint8[] get_address () {
			uint8[] address = new uint8[Consts.ADDRESS_SIZE];
		...
		public uint8[] get_public_key () {
			uint8[] pkey = new uint8[Consts.PUBLIC_KEY_SIZE];
		...
		public uint8[] get_secret_key () {
			uint8[] skey = new uint8[Consts.SECRET_KEY_SIZE];
	   ...
	   */
		private bool self_set_name (uint8[] name, out ToxErrors.TOX_ERR_SET_INFO error);
		public void set_name (string name)
			requires (name.data.length <= Consts.MAX_NAME_LENGTH)
		{
			ToxErrors.TOX_ERR_SET_INFO error;
			self_set_name (name.data, out error);
		}
		 ...
		 */
		private bool self_set_status_message (uint8[] status_message, out ToxErrors.TOX_ERR_SET_INFO error);
		public bool set_status_message (string status_message)
			requires (status_message.data.length <= Consts.MAX_STATUS_MESSAGE_LENGTH)
		{
			ToxErrors.TOX_ERR_SET_INFO error;
			return self_set_status_message (status_message.data, out error);
		}
	   ...
	   */
		...
		private bool _bootstrap (
			...
			out ToxErrors.TOX_ERR_BOOTSTRAP error
		);

		[CCode (cname="vala_tox_bootstrap")]
		public void bootstrap (string address, uint16 port, uint8[] public_key)
			throws BindingErrors.BootstrapError
		{
			ToxErrors.TOX_ERR_BOOTSTRAP error;
			bool result = _bootstrap (address, port, public_key, out error);

			if (!result) {
				switch (error) {
					case ToxErrors.TOX_ERR_BOOTSTRAP.BAD_HOST:
						throw new BindingErrors.BootstrapError.BAD_HOST("Host %s is invalid.".printf (address));
						break;
					case ToxErrors.TOX_ERR_BOOTSTRAP.BAD_PORT:
						throw new BindingErrors.BootstrapError.BAD_PORT("Port %d not valid or closed.".printf (port));
						break;
					default:
						throw new BindingErrors.BootstrapError.BAD_HOST("Unable to connect, please check bootstrap parameters");
						break;
				}
			}
		}
		...
		public void add_tcp_relay (string address, uint16 port, uint8[] public_key)
			throws BindingErrors.BootstrapError
			requires (public_key.length == Consts.PUBLIC_KEY_SIZE)
		{
			ToxErrors.TOX_ERR_BOOTSTRAP error;
			bool result = _add_tcp_relay (address, port, public_key, out error);

			if (!result) {
				switch (error) {
					case ToxErrors.TOX_ERR_BOOTSTRAP.BAD_HOST:
						throw new BindingErrors.BootstrapError.BAD_HOST("Relay Host %s is invalid.".printf (address));
						break;
					case ToxErrors.TOX_ERR_BOOTSTRAP.BAD_PORT:
						throw new BindingErrors.BootstrapError.BAD_PORT("Port %d not valid or closed.".printf (port));
						break;
					default:
						throw new BindingErrors.BootstrapError.BAD_HOST("Unable to connect, please check relay parameters");
						break;
				}
			}
		}
		...
		public Enums.ConnectionStatus get_connection_status ();
		public delegate void ConnectionStatusFunc (Tox handle, Enums.ConnectionStatus status);
		...
		private uint32 friend_add (
			...
			uint8[] message,
			out ToxErrors.TOX_ERR_FRIEND_ADD error
		);

		public uint32 add_friend (uint8[] address, string message)
			throws BindingErrors.FriendAddError
			requires (message.data.length <= Consts.MAX_FRIEND_REQUEST_LENGTH)
			requires (message.data.length > 0)
			requires (address.length == Consts.ADDRESS_SIZE)
		{
			ToxErrors.TOX_ERR_FRIEND_ADD error;
			uint32 friend_number = friend_add (address, message.data, out error);

			switch (error) {
				case ToxErrors.TOX_ERR_FRIEND_ADD.OWN_KEY:
					throw new BindingErrors.FriendAddError.OWNKEY ("The provided key is the same client key");
					break;
				case ToxErrors.TOX_ERR_FRIEND_ADD.ALREADY_SENT:
					throw new BindingErrors.FriendAddError.ALREADYSENT ("The provided key belongs to an already addede friend");
					break;
				case ToxErrors.TOX_ERR_FRIEND_ADD.BAD_CHECKSUM:
					throw new BindingErrors.FriendAddError.BADCHECKSUM ("Checksum was invalid");
					break;
				case ToxErrors.TOX_ERR_FRIEND_ADD.SET_NEW_NOSPAM:
					throw new BindingErrors.FriendAddError.SETNEWNOSPAM ("Client was already added, changed the nospam value");
					break;
				case ToxErrors.TOX_ERR_FRIEND_ADD.MALLOC:
					throw new BindingErrors.FriendAddError.NOMEM ("Error allocating the friend request");
					break;
				default:
					throw new BindingErrors.FriendAddError.UNKNOWN ("Unknown error, please check the request parameters");
					break;
			}
		...
		public uint32 add_friend_norequest (uint8[] public_key)
			throws BindingErrors.FriendAddError
		{
			ToxErrors.TOX_ERR_FRIEND_ADD error;
			uint32 friend_number = friend_add_norequest (public_key, out error);
		 ...
		 * @return true on success.
		 */
		private bool friend_delete (
			uint32 friend_number,
			out ToxErrors.TOX_ERR_FRIEND_DELETE error
		);

		public bool delete_friend (uint32 friend_number) {
			ToxErrors.TOX_ERR_FRIEND_DELETE error;
			return friend_delete (friend_number, out error);
		}
  	 ...
  	 */
		private uint32 friend_by_public_key (
			[CCode (array_length=false)] uint8[] public_key,
			out ToxErrors.TOX_ERR_FRIEND_BY_PUBLIC_KEY error
		);

		public uint32 get_friend_by_public_key (uint8[] public_key)
			throws BindingErrors.FriendGetError
		{
			ToxErrors.TOX_ERR_FRIEND_BY_PUBLIC_KEY error;
			uint32 friend_number = friend_by_public_key (public_key, out error);

			switch (error) {
				case ToxErrors.TOX_ERR_FRIEND_BY_PUBLIC_KEY.NOT_FOUND:
					throw new BindingErrors.FriendGetError.NOT_FOUND ("The provided public key doesn't belong to a friend.");
					break;
				default:
					break;
			}
		 ...
		 *   this parameter is NULL, this function has no effect.
		 */
		private void self_get_friend_list (
			[CCode (array_length = false)] uint32[] friend_list
		);
		...
		private bool friend_get_public_key (
			uint32 friend_number,
			[CCode (array_length=false)] uint8[] public_key,
			out ToxErrors.TOX_ERR_FRIEND_GET_PUBLIC_KEY error
		);

		public uint8[] get_friend_public_key (uint32 friend_number) {
			ToxErrors.TOX_ERR_FRIEND_GET_PUBLIC_KEY error;
			uint8[] retval = new uint8[Consts.PUBLIC_KEY_SIZE];
			bool friend_pkey = friend_get_public_key (friend_number, retval, out error);

			if (!friend_pkey) {
				throw new BindingErrors.FriendGetError.NOT_FOUND ("Friend number %u is not valid.".printf (friend_number));
			}
		 ...
		 */
		private uint64 friend_get_last_online (
			uint32 friend_number,
			out ToxErrors.TOX_ERR_FRIEND_GET_LAST_ONLINE error
		);

		public uint64 get_friend_last_online (uint32 friend_number)
			throws BindingErrors.FriendGetError
		{
			ToxErrors.TOX_ERR_FRIEND_GET_LAST_ONLINE error;
			uint64 retval = friend_get_last_online (friend_number, out error);

			if (retval == uint64.MAX) {
				throw new BindingErrors.FriendGetError.NOT_FOUND ("Unable to get the last seen time, friend_number %u seems invalid.".printf (friend_number));
			}
		 ...
		 */
		private size_t friend_get_name_size (
			uint32 friend_number,
			out ToxErrors.TOX_ERR_FRIEND_QUERY error
		);
		...
		public string get_friend_name (uint32 friend_number)
			throws BindingErrors.FriendGetError
		{
			ToxErrors.TOX_ERR_FRIEND_QUERY error;
			size_t size = friend_get_name_size (friend_number, out error);

			if (error == ToxErrors.TOX_ERR_FRIEND_QUERY.NOT_FOUND) {
				throw new BindingErrors.FriendGetError.NOT_FOUND ("Friend number %u is invalid".printf (friend_number));
			}

			uint8[] name = new uint8[size];
			bool val = friend_get_name (friend_number, name, out error);

			if (!val) {
				throw new BindingErrors.FriendGetError.UNKNOWN ("Unknown error on getting the name.");
			}
		...
		public delegate void FriendNameFunc (
			Tox handle,
			uint32 friend_number,
			uint8[] name
		);
		 ...
		 */
		private size_t friend_get_status_message_size (
			uint32 friend_number,
			out ToxErrors.TOX_ERR_FRIEND_QUERY error
		);
		...
		public string get_friend_status_message (uint32 friend_number)
			throws BindingErrors.FriendGetError
		{
			ToxErrors.TOX_ERR_FRIEND_QUERY error;
			size_t size = friend_get_status_message_size (friend_number, out error);

			if (error == ToxErrors.TOX_ERR_FRIEND_QUERY.NOT_FOUND) {
				throw new BindingErrors.FriendGetError.NOT_FOUND ("Friend number %u not found".printf (friend_number));
			}

			uint8[] retval = new uint8[size];
			bool val = friend_get_status_message (friend_number, retval, out error);

			if (!val) {
				throw new BindingErrors.FriendGetError.UNKNOWN ("Unknown error, please check get_friend_status_message params");
			}
		...
		public delegate void FriendStatusMessageFunc (
			Tox handle,
			uint32 friend_number,
			uint8[] message
		);
		 ...
		 */
		private Enums.UserStatus friend_get_status (
			uint32 friend_number,
			out ToxErrors.TOX_ERR_FRIEND_QUERY error
		);

		public Enums.UserStatus get_friend_status (uint32 friend_number)
			throws BindingErrors.FriendGetError
		{
			ToxErrors.TOX_ERR_FRIEND_QUERY error;
			Enums.UserStatus retval = friend_get_status (friend_number, out error);
		...
		public delegate void FriendStatusFunc (
			Tox handle,
			uint32 friend_number,
		 ...
		 */
		private Enums.ConnectionStatus friend_get_connection_status (
			uint32 friend_number,
			out ToxErrors.TOX_ERR_FRIEND_QUERY error
		);

		public Enums.ConnectionStatus get_friend_connection_status (uint32 friend_number)
			throws BindingErrors.FriendGetError
		{
			ToxErrors.TOX_ERR_FRIEND_QUERY error;
			Enums.ConnectionStatus retval = friend_get_connection_status (friend_number, out error);
		...
		public delegate void FriendConnectionStatusFunc (
			Tox handle,
			uint32 friend_number,
		 ...
		 */
		private bool friend_get_typing (
			uint32 friend_number,
			out ToxErrors.TOX_ERR_FRIEND_QUERY error
		);

		public bool friend_is_typing (uint32 friend_number)
			throws BindingErrors.FriendGetError
		{
			ToxErrors.TOX_ERR_FRIEND_QUERY error;
			bool retval = friend_get_typing (friend_number, out error);
		...
		public delegate void FriendTypingFunc (
			Tox handle,
			uint32 friend_number,
			bool is_typing
		);
		...
		public bool set_typing_for_friend (uint32 friend_number, bool is_typing) {
			ToxErrors.TOX_ERR_FRIEND_DELETE error;
			bool retval = self_set_typing (friend_number, is_typing, out error);
		...
		private uint32 friend_send_message (
			...
			uint8[] message,
			out ToxErrors.TOX_ERR_FRIEND_SEND_MESSAGE error
		);

		public uint32 send_friend_message (uint32 friend_number, Enums.MessageType type, string message)
			throws BindingErrors.SendMessageError
			requires (message.length > 0)
			requires (message.data.length < Consts.MAX_MESSAGE_LENGTH)
		{
			ToxErrors.TOX_ERR_FRIEND_SEND_MESSAGE error;
			uint32 retval = friend_send_message (friend_number, type, message.data, out error);

			switch (error) {
				case ToxErrors.TOX_ERR_FRIEND_SEND_MESSAGE.FRIEND_NOT_FOUND:
					throw new BindingErrors.SendMessageError.FRIEND_NOT_FOUND ("Friend number %u not found".printf (friend_number));
					break;
				case ToxErrors.TOX_ERR_FRIEND_SEND_MESSAGE.FRIEND_NOT_CONNECTED:
					throw new BindingErrors.SendMessageError.FRIEND_NOT_CONNECTED ("Friend number %u not connected, maybe try to queue it for later?".printf (friend_number));
					break;
				case ToxErrors.TOX_ERR_FRIEND_SEND_MESSAGE.SENDQ:
					throw new BindingErrors.SendMessageError.SENDQ ("Error increasing the message queue size for friend number %u".printf (friend_number));
				default:
					break;
			}
		...
		public delegate void ReadReceiptFunc(
			Tox handle,
			uint32 friend_number,
			uint32 message_id
		);
		...
		private bool _hash (
			[CCode (array_length=false)] uint8[] hash,
			uint8[] data
		);

		[CCode (cname="vala_tox_hash")]
		public uint8[]? hash (uint8[] data) {
			uint8[] retval = new uint8[Consts.HASH_LENGTH];
			_hash (retval, data);
			return retval;
		}
		...
		private bool _file_control (
			uint32 friend_number,
			uint32 file_number,
			Enums.FileControlStatus control,
			out ToxErrors.TOX_ERR_FILE_CONTROL error
		);

		[CCode (cname="vala_tox_file_control")]
		public void file_control (uint32 friend_number, uint32 file_number, Enums.FileControlStatus status)
			throws BindingErrors.FileControlError
		{
			ToxErrors.TOX_ERR_FILE_CONTROL error;
			bool res = _file_control (friend_number, file_number, status, out error);

			if (!res) {
				switch (error) {
					case ToxErrors.TOX_ERR_FILE_CONTROL.FRIEND_NOT_FOUND:
						throw new BindingErrors.FileControlError.FRIEND_NOT_FOUND ("Friend number %u not found".printf (friend_number));
						break;
					case ToxErrors.TOX_ERR_FILE_CONTROL.FRIEND_NOT_CONNECTED:
						throw new BindingErrors.FileControlError.FRIEND_NOT_CONNECTED ("Friend number %u is not connected".printf (friend_number));
						break;
					case ToxErrors.TOX_ERR_FILE_CONTROL.NOT_FOUND:
						throw new BindingErrors.FileControlError.NOT_FOUND ("File transfer ID %u not found".printf (file_number));
						break;
					case ToxErrors.TOX_ERR_FILE_CONTROL.NOT_PAUSED:
						throw new BindingErrors.FileControlError.NOT_PAUSED ("File transter ID %u not found".printf (file_number));
						break;
					case ToxErrors.TOX_ERR_FILE_CONTROL.DENIED:
						throw new BindingErrors.FileControlError.DENIED ("File transfer ID %u was paused by the other party".printf (file_number));
						break;
					case ToxErrors.TOX_ERR_FILE_CONTROL.ALREADY_PAUSED:
						throw new BindingErrors.FileControlError.ALREADY_PAUSED ("File transfer ID %u was already paused".printf (file_number));
						break;
					case ToxErrors.TOX_ERR_FILE_CONTROL.SENDQ:
						throw new BindingErrors.FileControlError.SENDQ ("Packet Queue is full, please retry later");
						break;
					default:
						break;
				}
		...
		public delegate void FileControlReceiveFunc (
			...
			uint32 friend_number,
			uint32 file_number,
			Enums.FileControlStatus status
		);
		...
		private bool _file_seek (
			...
			uint32 file_number,
			uint64 position,
			out ToxErrors.TOX_ERR_FILE_SEEK error
		);

		[CCode (cname = "vala_tox_file_seek")]
		public void file_seek (uint32 friend_number, uint32 file_number, uint16 position)
			throws BindingErrors.FileSeekError
		{
			ToxErrors.TOX_ERR_FILE_SEEK error;
			bool res = _file_seek (friend_number, file_number, position, out error);

			if (!res) {
				switch (error) {
					case ToxErrors.TOX_ERR_FILE_SEEK.FRIEND_NOT_FOUND:
						throw new BindingErrors.FileSeekError.FRIEND_NOT_FOUND ("Friend number %u not found".printf (friend_number));
						break;
					case ToxErrors.TOX_ERR_FILE_SEEK.FRIEND_NOT_CONNECTED:
						throw new BindingErrors.FileSeekError.FRIEND_NOT_CONNECTED ("Friend number %u is not connected".printf (friend_number));
					default:
						break;
				}
			}
		...
		private bool file_get_file_id (
			...
			out ToxErrors.TOX_ERR_FILE_GET error
		);

		public uint8[] get_file_id (uint32 friend_number, uint32 file_number)
			throws BindingErrors.FileGetError
		{
			ToxErrors.TOX_ERR_FILE_GET error;
			uint8[] retval = new uint8[Consts.FILE_ID_LENGTH];
			bool res = file_get_file_id (friend_number, file_number, retval, out error);

			if (!res) {
				switch (error) {
					case ToxErrors.TOX_ERR_FILE_GET.FRIEND_NOT_FOUND:
						throw new BindingErrors.FileGetError.FRIEND_NOT_FOUND ("Friend number %u not found".printf (friend_number));
						break;
					case ToxErrors.TOX_ERR_FILE_GET.NOT_FOUND:
						throw new BindingErrors.FileGetError.NOT_FOUND ("File number %u not valid".printf (file_number));
						break;
					default:
						break;
				}
		...
		private uint32 file_send (
			...
			[CCode (array_length=false)] uint8[]? file_id,
			uint8[] filename,
			out ToxErrors.TOX_ERR_FILE_SEND error
		);
		...
		public uint32 send_file (
			...
			Enums.FileKind kind,
			uint64 file_size,
			uint8[]? file_id = new uint8[Consts.FILE_ID_LENGTH],
			string filename
		)
			throws BindingErrors.FileSeekError
			requires (filename.data.length <= Consts.MAX_FILENAME_LENGTH)
		{
			ToxErrors.TOX_ERR_FILE_SEND error;
			uint32 retval = file_send (friend_number, kind, file_size, file_id, filename.data, out error);

			if (retval == uint32.MAX) {
				switch (error) {
					case ToxErrors.TOX_ERR_FILE_SEND.FRIEND_NOT_FOUND:
						throw new BindingErrors.FileSendError.FRIEND_NOT_FOUND ("Friend number %u not found".printf (friend_number));
						break;
					case ToxErrors.TOX_ERR_FILE_SEND.FRIEND_NOT_CONNECTED:
						throw new BindingErrors.FileSendError.FRIEND_NOT_CONNECTED ("Friend number %u is not online".printf (friend_number));
						break;
					case ToxErrors.TOX_ERR_FILE_SEND.TOO_MANY:
						throw new BindingErrors.FileSendError.TOO_MANY ("Too many outgoing connections for friend number %u".printf (friend_number));
						break;
					default:
						break;
				}
		...
		private bool file_send_chunk (
			...
			uint64 position,
			uint8[] data,
			out ToxErrors.TOX_ERR_FILE_SEND_CHUNK error
		);
		...
		public void send_file_chunk (
			...
			throws BindingErrors.FileSendChunkError
		{
			ToxErrors.TOX_ERR_FILE_SEND_CHUNK error;
			bool res = file_send_chunk (friend_number, file_number, position, data, out error);

			if (!res) {
				switch (error) {
					case ToxErrors.TOX_ERR_FILE_SEND_CHUNK.FRIEND_NOT_FOUND:
						throw new BindingErrors.FileSendChunkError.FRIEND_NOT_FOUND ("Friend number %u not found".printf (friend_number));
						break;
					case ToxErrors.TOX_ERR_FILE_SEND_CHUNK.FRIEND_NOT_CONNECTED:
						throw new BindingErrors.FileSendChunkError.FRIEND_NOT_CONNECTED ("Friend number %u is not online".printf (friend_number));
						break;
					case ToxErrors.TOX_ERR_FILE_SEND_CHUNK.NOT_FOUND:
						throw new BindingErrors.FileSendChunkError.NOT_FOUND ("File number %u not found".printf (file_number));
						break;
					case ToxErrors.TOX_ERR_FILE_SEND_CHUNK.NOT_TRANSFERRING:
						throw new BindingErrors.FileSendChunkError.NOT_TRANSFERRING ("Friend number %u is not acccepting the connection, check %u file number".printf (friend_number, file_number));
						break;
					case ToxErrors.TOX_ERR_FILE_SEND_CHUNK.INVALID_LENGTH:
						throw new BindingErrors.FileSendChunkError.INVALID_LENGTH ("Length %u is not valid for file number %u".printf (data.length, friend_number));
						break;
					case ToxErrors.TOX_ERR_FILE_SEND_CHUNK.SENDQ:
						throw new BindingErrors.FileSendChunkError.SENDQ ("Packet queue full for file number %u and friend number %u".printf (file_number, friend_number));
						break;
